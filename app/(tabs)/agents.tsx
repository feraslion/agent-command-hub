...
type PromptTemplateKey = "planner" | "coder" | "qa";
type TemplateOption = { key: PromptTemplateKey; title: string; arabicTitle: string; description: string; documentPath: string };
type PromptAssignment = { agentKey: string; templateKey: PromptTemplateKey; customInstructions: string };

const fallbackTemplates: TemplateOption[] = [
  { key: "planner", title: "Planner", arabicTitle: "المخطط", description: "خطة، تبعيات، ومعايير قرار واضحة.", documentPath: "docs/prompts/planner-system-prompt-ar.md" },
  { key: "coder", title: "Coder", arabicTitle: "المبرمج", description: "مسودات مقيدة ومراجعة فروقات قبل الحفظ.", documentPath: "docs/prompts/coder-system-prompt-ar.md" },
  { key: "qa", title: "QA", arabicTitle: "مختبر الجودة", description: "تحقق منطقي وملاحظات قابلة للتتبع.", documentPath: "docs/prompts/qa-system-prompt-ar.md" },
];
...
export default function AgentsScreen() {
  ...
  const saveMutation = trpc.agentPrompts.save.useMutation({
    onSuccess: () => utils.agentPrompts.list.invalidate(),
  });
  ...
  return (
    <ScreenContainer className="px-5" containerClassName="bg-[#F7F7FC]">
      <FlatList
        ...
        renderItem={({ item }) => <AgentCard agent={item} template={assignmentByAgent.get(item.id)?.templateKey ?? defaultTemplateForAgent(item.id)} onPress={() => setSelectedAgent(item)} />}
...
function AgentDetail({ agent, assignment, templates, loading, saving, saveError, onSave, onClose }: {
  ...
  saving: boolean;
  saveError?: string;
  onSave: (input: { agentKey: string; templateKey: PromptTemplateKey; customInstructions: string }) => void;
  onClose: () => void;
}) {
  ...
  }, [agent.id, assignment?.templateKey, assignment?.customInstructions]);

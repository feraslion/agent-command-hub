...
function AgentDetail({ agent, assignment, templates, loading, saving, saveError, onSave, onClose }: {
  ...
  const commandMutation = trpc.agentCommands.create.useMutation({ onSuccess: (command) => setCommandNotice(`حُفظ طلب ${command.intent} للمراجعة؛ لم يُنفذ أي بناء أو أداة.`), onError: (error) => setCommandNotice(error.message || "تعذر حفظ أمر الوكيل.") });

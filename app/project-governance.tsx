...
type SectionKey = "brief" | "plan" | "evidence";
...
export default function ProjectGovernanceScreen() {
  ...
  const isPending = saveBrief.isPending || createPlan.isPending || createTask.isPending || createCriterion.isPending || addDependency.isPending || registerArtifact.isPending || createContextPackage.isPending || createReport.isPending;

  const selectedTask = useMemo(() => tasks.find((task) => task.id === selectedTaskId) ?? null, [tasks, selectedTaskId]);
  const error = governanceQuery.error ?? tasksQuery.error ?? projectQuery.error;
  ...
  return (
    <ScreenContainer className="px-5" containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {saveBrief.error || createPlan.error || createTask.error || createCriterion.error || addDependency.error || registerArtifact.error || createContextPackage.error || createReport.error ? <View style={[styles.error, { backgroundColor: "#4A202A", borderColor: "#6C2C3B" }]}><Text style={styles.errorText}>تعذر حفظ التعديل. راجع الحقول، وتأكد أن الاعتماد لا يشكل دورة أو أن حزمة السياق تتضمن مرجعاً واحداً على الأقل.</Text></View> : null}
...
function EvidenceSection({ colors, tasks, activePlan, briefExists, artifacts, contextPackages, reports, timeline, selectedTaskId, selectedArtifactIds, artifactName, artifactKind, artifactReference, artifactSummary, contextTitle, pending, onSetArtifactName, onSetArtifactKind, onSetArtifactReference, onSetArtifactSummary, onToggleArtifact, onSetContextTitle, onRegisterArtifact, onCreateContext, onCreateReport }: { colors: ReturnType<typeof useColors>; tasks: any[]; activePlan: any; briefExists: boolean; artifacts: any[]; contextPackages: any[]; reports: any[]; timeline: any[]; selectedTaskId: number | null; selectedArtifactIds: number[]; artifactName: string; artifactKind: string; artifactReference: string; artifactSummary: string; contextTitle: string; pending: boolean; onSetArtifactName: (value: string) => void; onSetArtifactKind: (value: string) => void; onSetArtifactReference: (value: string) => void; onSetArtifactSummary: (value: string) => void; onToggleArtifact: (id: number) => void; onSetContextTitle: (value: string) => void; onRegisterArtifact: () => void; onCreateContext: () => void; onCreateReport: (kind: "delivery" | "blocked", finalize: boolean) => void }) {

    ...
    <ScreenContainer className="px-5" containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Section title="طلب بناء للمراجعة" colors={colors}>
          ...
          <Pressable disabled={busy || !latestImportId || !selectedTemplate?.targets.includes(target)} onPress={() => buildMutation.mutate({ projectId, importId: latestImportId, target, templateKey, title, summary })} style={({ pressed }) => [styles.primaryButton, { backgroundColor: colors.primary }, (pressed || busy || !latestImportId || !selectedTemplate?.targets.includes(target)) && styles.pressed]}><Text style={styles.primaryText}>{buildMutation.isPending ? "جارٍ إنشاء الطلب…" : latestImportId ? "إنشاء طلب بناء للموافقة" : "أضف مصدراً أولاً"}</Text></Pressable>
        </Section>

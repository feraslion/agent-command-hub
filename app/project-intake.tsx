    ...
    <ScreenContainer className="px-5" containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Section title="تسجيل مرجع مستودع" colors={colors}>
          ...
          <Pressable disabled={busy || !remoteUrl.trim()} onPress={() => repositoryMutation.mutate({ projectId, remoteUrl, defaultBranch: branch || "main" })} style={({ pressed }) => [styles.outlineButton, { borderColor: colors.primary }, (pressed || busy || !remoteUrl.trim()) && styles.pressed]}><Text style={[styles.outlineText, { color: colors.primary }]}>{repositoryMutation.isPending ? "جارٍ التسجيل…" : "تسجيل المرجع فقط"}</Text></Pressable>
        </Section>

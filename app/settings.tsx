...
function ManagedWorkerControl() {
  ...
  return <><View><Text style={styles.section}>العامل المُدار</Text><View style={styles.group}><SettingRow title="استقبال أوامر التشغيل" description="يحفظ طلبك لتبدأ معالجة الأوامر تلقائياً عند توفر الخدمة الدائمة." value={enabled} disabled={workerQuery.isLoading || workerMutation.isPending} onChange={(value) => workerMutation.mutate({ enabled: value })} /><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>{workerQuery.error ? "تعذر تحميل الحالة" : statusText}</Text><Text style={styles.statusLabel}>حالة العامل</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>{heartbeat}</Text><Text style={styles.statusLabel}>آخر heartbeat</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>{status?.loop.configured ? `كل ${Math.round(status.loop.intervalMs / 1000)} ثوانٍ` : "معطّلة إعدادياً"}</Text><Text style={styles.statusLabel}>حلقة العامل الجاف</Text></View>{enabled && worker?.runtimeStatus !== "ready" ? <Text style={styles.workerNote}>تم تفعيل استقبال الأوامر داخل التطبيق. سيبدأ التنفيذ عندما يتصل عامل الخدمة الدائمة ويرسل heartbeat.</Text> : null}{enabled && worker?.runtimeStatus === "ready" ? <Text style={styles.workerNote}>العامل متصل: يحجز أمراً واحداً لكل مالك، ويتركه في حالة محجوزة. لا ينفذ أدوات أو أوامر نظام في هذه المرحلة.</Text> : null}</View></View><LocalRunnerControl /></>;
}

function LocalRunnerControl() {
  const utils = trpc.useUtils();
  const [label, setLabel] = useState("جهاز التطوير");
  const [pairing, setPairing] = useState<{ runner: { runnerKey: string }; token: string } | null>(null);
  const runnersQuery = trpc.localRunners.list.useQuery(undefined, { refetchInterval: 8_000 });
  const runtimeStatus = trpc.isolatedRuntime.status.useQuery(undefined, { refetchInterval: 8_000 });
  const createPairing = trpc.localRunners.createPairing.useMutation({ onSuccess: (data) => { setPairing(data); utils.localRunners.list.invalidate(); } });
  const revokeRunner = trpc.localRunners.revoke.useMutation({ onSuccess: () => { setPairing(null); utils.localRunners.list.invalidate(); } });
  const runner = runnersQuery.data?.[0];
  const readiness = getRunnerReadiness(runner ?? {});
  const lastHeartbeat = runner?.lastHeartbeatAt ? new Date(runner.lastHeartbeatAt).toLocaleString("ar-SA", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "لم يصل بعد";
  return <View><Text style={styles.section}>Runner المحلي المعزول</Text><View style={styles.group}><View style={styles.budgetEditor}><Text style={styles.budgetTitle}>{runtimeStatus.data?.label ?? "جارٍ فحص الحالة"}</Text><Text style={styles.budgetDescription}>{runtimeStatus.data?.detail ?? "لا يبدأ أي تنفيذ قبل موافقتك وظهور heartbeat صالح من جهاز يملك Docker."}</Text></View>{runner ? <View><View style={styles.divider} /><View style={styles.statusRow}><Text style={[styles.statusValue, readiness.canAcceptWork ? styles.feedbackSuccess : styles.feedbackError]}>{readiness.connectionLabel}</Text><Text style={styles.statusLabel}>اتصال Runner</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>{readiness.dockerLabel}</Text><Text style={styles.statusLabel}>Docker المقيد</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>JavaScript: {readiness.javascriptLabel} · TypeScript: {readiness.typescriptLabel}</Text><Text style={styles.statusLabel}>قدرات التنفيذ</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text style={styles.statusValue}>{lastHeartbeat}</Text><Text style={styles.statusLabel}>آخر heartbeat</Text></View><View style={styles.divider} /><View style={styles.statusRow}><Text selectable style={styles.statusValue}>{runner.runnerKey}</Text><Text style={styles.statusLabel}>المعرّف</Text></View><Text style={styles.runnerTruth}>{readiness.canAcceptWork ? "تظهر الجاهزية لأن الجهاز أرسل heartbeat وقدراته بعد فحصه المحلي. يبقى كل تنفيذ محمياً بموافقة صريحة." : "لا تعني بيانات الإقران وحدها أن Docker صالح أو أن التنفيذ متاح؛ يلزم heartbeat حديث من العميل المحلي."}</Text><Pressable onPress={() => revokeRunner.mutate({ runnerId: runner.id })} disabled={revokeRunner.isPending} style={({ pressed }) => [styles.saveButton, pressed && styles.pressed]}><Text style={styles.saveButtonText}>{revokeRunner.isPending ? "جارٍ الإلغاء…" : "إلغاء Runner"}</Text></Pressable></View> : <View style={styles.budgetEditor}><TextInput value={label} onChangeText={setLabel} maxLength={128} placeholder="اسم جهازك" placeholderTextColor="#9BA0AF" style={styles.input} textAlign="right" /><Pressable disabled={label.trim().length < 2 || createPairing.isPending} onPress={() => createPairing.mutate({ label: label.trim() })} style={({ pressed }) => [styles.saveButton, pressed && styles.pressed]}><Text style={styles.saveButtonText}>{createPairing.isPending ? "جارٍ إنشاء الرمز…" : "إنشاء رمز إقران"}</Text></Pressable></View>}{pairing ? <View style={styles.note}><Text style={styles.noteTitle}>الرمز يعرض الآن فقط</Text><Text selectable style={styles.statusValue}>Runner: {pairing.runner.runnerKey}</Text><Text selectable style={styles.statusValue}>Token: {pairing.token}</Text><Text style={styles.noteCopy}>استخدمه عند تشغيل العميل المحلي، ولا تحفظه في Git أو ملف مشترك.</Text></View> : null}</View></View>;
}
...
const styles = StyleSheet.create({
  ...
  workerNote: { color: "#6C629A", fontSize: 11, lineHeight: 17, paddingHorizontal: 15, paddingBottom: 14, textAlign: "right" },
  runnerTruth: { color: "#6C629A", fontSize: 11, lineHeight: 17, paddingHorizontal: 15, paddingTop: 14, textAlign: "right" },

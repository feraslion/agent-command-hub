...
import { applySavedMultiFileTemplate, hasValidManualMultiFileBundle, isEligibleMultiFilePath, toggleMultiFileSelection } from "@/lib/multi-file-bundle-selection";
...
export default function WorkspaceBrowserScreen() {
  ...
  const sensitivity = useMemo(() => assessSensitiveWorkspaceChange(selectedPath ?? "", selectedFile?.content ?? "", draft), [selectedPath, selectedFile?.content, draft]);
  const activeSensitiveChange = sensitiveChanges.data?.find((change) => change.path === selectedPath && change.status === "pending_secondary");

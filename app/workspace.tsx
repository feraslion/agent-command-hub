...
import { buildLineDiff, languageFromPath, languageLabel, summarizeDiff, tokenizeCodeLine, type DiffLine } from "@/lib/workspace-editor";
...
export default function WorkspaceBrowserScreen() {
  ...
  const writeFile = trpc.workspace.writeFile.useMutation({
    onSuccess: () => {
      ...
      utils.workspace.readFile.invalidate({ projectId, path: selectedPath ?? "source/placeholder" });
  ...
  const selectedFileVersion = selectedFile?.version;
  const language = languageFromPath(selectedPath ?? "");
  const diffLines = useMemo(() => buildLineDiff(selectedFile?.content ?? "", draft), [selectedFile?.content, draft]);
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><FlatList<WorkspaceFile>
    ...
    renderItem={({ item }) => <Pressable onPress={() => chooseFile(item.path)} style={({ pressed }) => [styles.fileRow, { backgroundColor: colors.surface, borderColor: selectedPath === item.path ? colors.primary : colors.border }, pressed && styles.pressed]}><View style={styles.fileMeta}><Text style={[styles.fileVersion, { color: colors.primary }]}>v{item.version}</Text><Text style={[styles.fileDate, { color: colors.muted }]}>{formatDate(item.updatedAt)}</Text></View><View style={styles.fileName}><Text style={[styles.path, { color: colors.foreground }]}>{item.path}</Text><Text style={[styles.fileSub, { color: colors.muted }]}>{languageLabel(languageFromPath(item.path))} · ملف افتراضي محفوظ</Text></View></Pressable>}
    ItemSeparatorComponent={() => <View style={styles.separator} />}
    ListHeaderComponent={<WorkspaceHeader router={router} colors={colors} projectName={params.projectName ?? "المشروع"} directories={directories} />}

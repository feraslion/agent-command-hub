...
بعد تطبيق التجاوز، أصبحت PRs #1 و#2 و#3 بحالة `CLEAN` وقابلة للدمج، مع نجاح فحص `TypeScript, tests, lint, and build`. دُمجت لاحقاً بالتسلسل وبطريقة squash: #1 في `72d0ec4`، و#2 في `2c91d22`، و#3 في `14533a8`. أصبح عدد طلبات السحب المفتوحة صفراً، وتتوفر الآن على `main` قوالب PR وIssues وسير تسمية Issues. يجب إعادة شرط الموافقة المستقلة إلى `1` فور توفر مراجع موثوق بصلاحية كتابة أو بعد اتخاذ قرار حوكمة بديل موثق.

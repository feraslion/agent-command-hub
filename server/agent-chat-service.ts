...
export async function runAgentChat(message: string, operations: ChatOperations = { listModels: listLLMModels, invoke: invokeLLM }, externalReference = "") {
  ...
  const response = await operations.invoke({ model, maxTokens: 800, messages: buildAgentChatMessages(message, externalReference) });
